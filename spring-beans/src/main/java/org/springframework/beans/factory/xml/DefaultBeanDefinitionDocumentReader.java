...
public class DefaultBeanDefinitionDocumentReader implements BeanDefinitionDocumentReader {
	...
	protected void processBeanDefinition(Element ele, BeanDefinitionParserDelegate delegate) {
		...
		// 得到这个bteanDefinitionHolder就意味着beanDefinition是通过BeanDefinitionParserDelegate对xml元素的信息按照spring的
		// bean规则进行解析得到的
		...
		if (bdHolder != null) {
			...
			try {
				...
				BeanDefinitionReaderUtils.registerBeanDefinition(bdHolder, getReaderContext().getRegistry());//（2）注册。☆
			}
			catch (BeanDefinitionStoreException ex) {
